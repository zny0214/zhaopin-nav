...
const features = [
  {
    title: '弹性伸缩',
    description: '基于业务负载自动调整计算资源，秒级扩容，轻松应对流量峰值，节约成本',
    details: '秒级扩容 | 自动缩容 | 成本优化 | 流量调度'
  },
  {
    title: '容器编排',
    description: 'Kubernetes内核的容器管理平台，简化微服务部署，实现资源高效利用',
    details: 'K8s原生 | 自动部署 | 服务发现 | 负载均衡'
  },
  {
    title: '微服务治理',
    description: '完整的服务治理体系，包括服务发现、负载均衡、熔断限流等核心能力',
    details: '服务注册 | 熔断器 | 限流 | 链路追踪'
  },
  {
    title: '安全防护',
    description: '多层次安全防护体系，包括网络安全、身份认证、数据加密等全方位保障',
    details: 'DDoS防护 | 身份认证 | 数据加密 | 漏洞扫描'
  }
]

const scenarios = [
  { name: '私有化部署', icon: '◈', desc: '满足数据本地化需求' },
  { name: '混合云架构', icon: '◇', desc: '跨云统一管理' },
  { name: '容器化改造', icon: '◉', desc: '传统应用容器化' },
  { name: 'DevOps 实践', icon: '◎', desc: 'CI/CD持续交付' }
]

const metrics = [
  { value: '99.99%', label: '服务可用性' },
  { value: '10x', label: '资源利用率提升' },
  { value: '50%', label: '运维成本降低' },
  { value: '秒级', label: '弹性扩容时间' }
]

const tabs = [
  { title: '计算资源', desc: 'GPU实例 | 内存优化 | 突发性能' },
  { title: '存储方案', desc: 'SSD云盘 | 文件存储 | 对象存储' },
  { title: '网络连接', desc: 'VPC专网 | 负载均衡 | CDN加速' },
  { title: '安全服务', desc: '防火墙 | 证书服务 | 入侵检测' }
]
  ...
  <div class="service-detail" :class="{ loaded: isLoaded }">
    ...
    <section class="hero">
      ...
      <div class="hero-content">
        <span class="hero-overline" :class="{ visible: isLoaded }">云计算平台</span>
        <h1 class="hero-title">
          <span class="hero-title-line" :class="{ visible: isLoaded }">云翼计算</span>
          <span class="hero-title-line accent" :class="{ visible: isLoaded }">新一代云基础设施</span>
        </h1>
        <p class="hero-description" :class="{ visible: isLoaded }">
          弹性可扩展的云端基础设施，支持私有化部署与混合云架构。从容器化改造到DevOps实践，全方位赋能企业云转型。
        </p>
    ...
    <section class="tabs-section">
      <div class="section-header">
        <span class="section-overline">云端资源</span>
        <h2 class="section-title">全方位云计算解决方案</h2>
      </div>
    ...
    <section class="scenarios">
      <div class="scenarios-content">
        <div class="scenarios-text">
          ...
          <h2 class="section-title">满足多元部署需求</h2>
          <p class="scenarios-desc">无论是需要数据本地化的私有化部署，还是追求灵活性的混合云架构，云翼计算都能提供最适合的解决方案。</p>
        </div>
...
.hero-bg {
  position: absolute;
  inset: 0;
  background: radial-gradient(ellipse at 50% 0%, rgba(90, 143, 205, 0.15) 0%, transparent 60%);
}
